 ...
 * @return array
 */
function slimpy_page_menu_args( $args ) {
  $args['show_home'] = true;
  return $args;
}
add_filter( 'wp_page_menu_args', 'slimpy_page_menu_args' );
 ...
 * @return array
 */
function slimpy_body_classes( $classes ) {
...
add_filter( 'body_class', 'slimpy_body_classes' );


// Mark Posts/Pages as Untiled when no title is used
add_filter( 'the_title', 'slimpy_title' );

function slimpy_title( $title ) {
  if ( $title == '' ) {
    return 'Untitled';
  } else {
    return $title;
...
function custom_password_form() {
  ...
  <div class="row">
    <div class="col-lg-10">
        <p>' . esc_html__( "This post is password protected. To view it please enter your password below:" ,'slimpy') . '</p>
        <label for="' . $label . '">' . esc_html__( "Password:" ,'slimpy') . ' </label>
      <div class="input-group">
        <input class="form-control" value="' . get_search_query() . '" name="post_password" id="' . $label . '" type="password">
        <span class="input-group-btn"><button type="submit" class="btn btn-default" name="submit" id="searchsubmit" value="' . esc_attr__( "Submit",'slimpy' ) . '">' . esc_html__( "Submit" ,'slimpy') . '</button>
        </span>
      </div>
    </div>
  </div>
...
add_filter( 'the_content', 'slimpy_add_custom_table_class' );
function slimpy_add_custom_table_class( $content ) {
  return preg_replace( '/(<table) ?(([^>]*)class="([^"]*)")?/', '$1 $3 class="$4 table table-hover" ', $content);
}


if ( ! function_exists( 'slimpy_social_icons' ) ) :
...
if ( ! function_exists( 'slimpy_header_menu' ) ) :
/**
 * Header menu (should you choose to use one)
 */
function slimpy_header_menu() {
...
if ( ! function_exists( 'slimpy_footer_links' ) ) :
/**
 * Footer menu (should you choose to use one)
 */
function slimpy_footer_links() {
  // display the WordPress Custom Menu if available
  wp_nav_menu(array(
    'container'       => '',                              // remove nav container
    'container_class' => 'footer-links clearfix',   // class of container (should you choose to use it)
    'menu'            => esc_html__( 'Footer Links', 'slimpy' ),   // nav name
    ...
    'fallback_cb'     => 'slimpy_footer_links_fallback'  // fallback function
  ));
} /* end slimpy footer link */
endif;
...
if ( ! function_exists( 'slimpy_featured_slider' ) ) :
/**
 * Featured image slider, displayed on front page for static page and blog
 */
function slimpy_featured_slider() {
  if ( is_front_page() && of_get_option( 'slimpy_slider_checkbox' ) == 1 ) {
    echo '<div class="flexslider">';
      echo '<ul class="slides">';

        $count = of_get_option( 'slimpy_slide_number' );
        $slidecat =of_get_option( 'slimpy_slide_categories' );
...
if ( ! function_exists( 'get_slimpy_theme_options' ) ) {
/**
 * Get information from Theme Options and add it into wp_head
 */
    function get_slimpy_theme_options(){
...
add_action( 'wp_head', 'get_slimpy_theme_options', 10 );

// Theme Options sidebar
add_action( 'optionsframework_after', 'slimpy_options_display_sidebar' );

function slimpy_options_display_sidebar() { ?>
  <!-- Twitter -->
  ...
  <div id="optionsframework-sidebar" class="metabox-holder">
    <div id="optionsframework" class="postbox">
        <h3><?php esc_html_e('Support and Documentation','slimpy') ?></h3>
        <div class="inside">
          <div id="social-share">
 ...
 */
function slimpy_caption($output, $attr, $content) {
  if (is_feed()) {
    return $output;
  }
...
add_filter('img_caption_shortcode', 'slimpy_caption', 10, 3);

/**
 * Skype URI support for social media icons
 */
function slimpy_allow_skype_protocol( $protocols ){
    $protocols[] = 'skype';
    return $protocols;
}
add_filter( 'kses_allowed_protocols' , 'slimpy_allow_skype_protocol' );

/**
 * Fallback option for the old Social Icons.
 */
function slimpy_social(){
...
/**
 * Fallback for removed slimpy_post_nav function
 */
if (!function_exists('slimpy_post_nav')) {
  function slimpy_post_nav() {
...
/**
 * Fallback for removed slimpy_paging_nav function
 */
if (!function_exists('slimpy_paging_nav')) {
  function slimpy_paging_nav() {
    the_posts_pagination( array(
        'prev_text' => '<i class="fa fa-chevron-left"></i> ' . __( 'Newer posts', 'slimpy' ),
        'next_text' => __( 'Older posts', 'slimpy' ) . ' <i class="fa fa-chevron-right"></i>' ,
    ) );
  }
}
 ...
 */
function  slimpy_add_top_level_menu_url( $atts, $item, $args ){
  if ( !wp_is_mobile() && isset($args->has_children) && $args->has_children  ) {
    $atts['href'] = ! empty( $item->url ) ? $item->url : '';
  }
  return $atts;
}
add_filter( 'nav_menu_link_attributes', 'slimpy_add_top_level_menu_url', 99, 3 );

/**
 * Makes the top level navigation menu item clickable
 */
function slimpy_make_top_level_menu_clickable(){
if ( !wp_is_mobile() ) { ?>
...
add_action('wp_footer', 'slimpy_make_top_level_menu_clickable', 1);
